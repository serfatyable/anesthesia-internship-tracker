...
describe('RotationCard', () => {
  ...
  it('shows intern count', () => {
    ...
    expect(screen.getByText('Interns in rotation:')).toBeInTheDocument();
  });
  ...
  it('displays no requirements notice when required is 0', () => {
    ...
    expect(screen.getByText('No specific requirements defined for this rotation.')).toBeInTheDocument();
  });

  it('applies custom className', () => {
    const { container } = render(<RotationCard rotation={mockRotation} className="custom-class" />);
  ...
  it('applies default background color for unknown rotation types', () => {
    const unknownRotation = { ...mockRotation, rotationName: 'Unknown Rotation' };
