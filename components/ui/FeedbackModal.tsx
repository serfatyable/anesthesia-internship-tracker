...
export const FeedbackModal = memo(function FeedbackModal({
  ...
  const loadFeedback = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/mentor-feedback?itemId=${itemId}&itemType=${itemType}`);
      if (response.ok) {
        const data = await response.json();
        setFeedbackData(data);
        // Auto-expand mentors with unread feedback
        const mentorsWithUnread = data.feedback
          .filter((group: MentorGroup) => group.feedback.some((f: FeedbackItem) => !f.isRead))
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex justify-center items-center z-50">
      <div className="bg-white p-6 rounded-lg shadow-xl max-w-2xl w-full m-4 max-h-[80vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Mentor Feedback</h2>
          ...
          {loading ? (
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="text-sm text-gray-600">
                      <span className="font-medium">{feedbackData.totalCount}</span> total feedback
                    </div>
              ...
              {feedbackData.feedback.map((group) => (
                <div key={group.mentor.id} className="border border-gray-200 rounded-lg">
                  {/* Mentor Header */}
                  <button
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-sm text-gray-600">{group.mentor.email}</p>
                      </div>
                    </div>
