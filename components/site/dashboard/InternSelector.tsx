...
export function InternSelector({ interns, selectedInternId, className = '' }: InternSelectorProps) {
  ...
  if (interns.length === 0) {
    return <div className={`text-sm text-gray-500 ${className}`}>No interns available</div>;
  }
    ...
    <div className={`relative ${className}`} data-intern-selector>
      ...
      >
        <span className="truncate">
          {selectedIntern ? selectedIntern.name || selectedIntern.email : 'Select an intern...'}
        </span>
        <svg
            ...
            {interns.map((intern) => (
              <li
                ...
                className={`px-4 py-2 text-sm cursor-pointer hover:bg-gray-100 ${
                  intern.id === selectedInternId ? 'bg-blue-50 text-blue-700' : 'text-gray-900'
                }`}
