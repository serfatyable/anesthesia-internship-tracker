...
export const FeedbackNotificationsCard = memo(function FeedbackNotificationsCard({
  className,
    ...
    const fetchNotifications = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.error('Error fetching feedback notifications:', response.status);
        }
      } catch (error) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">
              {unreadCount > 0 ? `${unreadCount} new feedback` : 'No new feedback'}
            </p>
          </div>
        </div>
        <svg
