...
const InternDashboard = dynamic(
  () =>
    import('@/components/site/dashboard/InternDashboard').then(
      mod => mod.InternDashboard
    ),
  {
    loading: () => (
...
const TutorDashboard = dynamic(
  () =>
    import('@/components/site/dashboard/TutorDashboard').then(
      mod => mod.TutorDashboard
    ),
  {
    loading: () => (
      <div className='flex items-center justify-center h-64'>
  ...
  if (!dashboardData || !dashboardData.summary) {
    return (
      <div className='bg-red-100 border border-red-300 text-red-700 p-6 rounded-lg text-center'>
        ...
        <p>
          Intern dashboard data is missing or invalid. Please contact support or
          try again later.
        </p>
      </div>
    );
