...
const BarChart = dynamic(() => import('./BarChart'), {
  loading: () => <div className="h-20 flex items-center justify-center">Loading chart...</div>,
  ssr: false,
});
...
function mockFetchTutors(page: number, limit: number) {
  ...
  return new Promise<{ tutors: typeof all; total: number }>((resolve) => {
    setTimeout(() => resolve({ tutors: all.slice(start, start + limit), total: all.length }), 500);
  });
}
...
export default function AnalyticsDashboard() {
  ...
  const atRisk = interns.filter((i) => rotations.some((r) => (i.progress[r] || 0) < 50));
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="bg-white border rounded-lg p-4">
                  <div className="text-sm text-zinc-500 mb-1">Total Interns</div>
                  <div className="text-2xl font-bold">{internTotal}</div>
                </div>
                <div className="bg-white border rounded-lg p-4">
                  <div className="text-sm text-zinc-500 mb-1">Fully Completed</div>
                  <div className="text-2xl font-bold">
                    {
                      interns.filter((i) => rotations.every((r) => (i.progress[r] || 0) === 100))
                        .length
                    }
                  </div>
                </div>
                <div className="bg-white border rounded-lg p-4">
                  <div className="text-sm text-zinc-500 mb-1">At-Risk Interns</div>
          ...
          {loading ? (
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div className="bg-white border rounded-lg p-4">
                  <div className="text-sm text-zinc-500 mb-1">Total Intern Assignments</div>
                  <div className="text-2xl font-bold">
              ...
              <div className="bg-white border rounded-lg p-4 mb-8">
                ...
                <BarChart data={tutors.map((t) => t.interns)} labels={tutors.map((t) => t.name)} />
              </div>
              <div className="bg-white border rounded-lg p-4">
                <div className="font-semibold mb-2">Feedback Given per Tutor</div>
                <BarChart
