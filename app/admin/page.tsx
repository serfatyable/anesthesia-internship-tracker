...
export default function AdminPage() {
  ...
  const handleAssignTutor = (
    tutorId: string,
    intern: { id: string; tutorIds: string[] }
  ) => {
    setInternList(prev =>
      prev.map(i =>
  ...
  const removeTutor = (internId: string, tutorId: string) => {
    setInternList(prev =>
      prev.map(i =>
        i.id === internId
          ? { ...i, tutorIds: i.tutorIds.filter(tid => tid !== tutorId) }
          : i
      )
    );
  };
  ...
  const removeInternFromRotation = (internId: string) => {
    setInternList(prev =>
      prev.map(i => (i.id === internId ? { ...i, rotationId: null } : i))
    );
  };
      ...
      <main className='flex-1 p-6'>
              ...
              .map(intern => (
                <div
                  ...
                  onClick={e => {
                    ...
                    if ((e.target as HTMLElement).closest('.remove-intern-x'))
                      return;
                    setAssignModal({
                      open: true,
                      type: 'intern',
                ...
                >
                  ...
                  <div className='text-xs text-zinc-700 text-center mb-1'>
                    {intern.name}
                  </div>
                  <button
                    onClick={e => {
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
          {rotations.map(rotation => (
            <div
              key={rotation.id}
              className={`flex-1 min-w-[260px] max-w-xs transition-all`}
            >
              <div
                  ...
                  .map(intern => (
                    <div
                      ...
                      onClick={e => {
                        // Only open modal if not clicking the x
                        if (
                          (e.target as HTMLElement).closest('.remove-intern-x')
                        )
                          return;
                        setAssignModal({
                          open: true,
        ...
        <AssignModal
          ...
          onClose={() =>
            setAssignModal({ open: false, type: '', targetId: null })
          }
          title={
          ...
          onAssign={
            assignModal.type === 'tutor'
              ? tutorId =>
                  ...
                  handleAssignTutor(tutorId, {
                    id: assignModal.targetId,
                    tutorIds: [],
                  })
              : assignModal.type === 'intern'
                ? rotationId =>
                    assignModal.targetId &&
                    handleAssignRotation(assignModal.targetId, {
                      id: rotationId,
                    })
                : () => {}
          }
