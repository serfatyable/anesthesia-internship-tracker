...
const rotations = [
  {
    id: 'icu',
    name: 'ICU',
    description: 'Intensive Care Unit rotation focusing on critical care procedures and knowledge',
    procedures: 12,
    knowledge: 8,
    completed: 8,
    total: 20,
          ...
          {rotations.map((rotation) => (
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-semibold text-gray-900">{rotation.name}</h3>
              ...
              <div className="space-y-2">
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    ...
                    style={{ width: `${(rotation.completed / rotation.total) * 100}%` }}
                  ></div>
                </div>

                <div className="text-xs text-gray-500 text-center">
                  {Math.round((rotation.completed / rotation.total) * 100)}% Complete
                </div>
              </div>
            </Link>
          ))}
